...
export default function Silent() {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get<StSCardVote[]>("https://sts-api.vercel.app/api/vote?class=Silent")
  ...
  async function submitVote(upvote: number, downvote: number) {
    axios.post(
      `https://sts-api.vercel.app/api/vote?class=Silent&upvoteId=${upvote}&downvoteId=${downvote}`
    );
    refreshCards();
  }
    ...
    <>
      ...
      <div className="flex h-[calc(100vh-3rem)] w-[100vw] flex-col items-center justify-between">
        ...
        <div className="hoverAnimation absolute bottom-24 flex h-10 w-32 items-center justify-center rounded-full bg-neutral-900 text-lg">
          <Link href="/Silent/Results">See results</Link>{" "}
        </div>
      </div>
    </>
  );
